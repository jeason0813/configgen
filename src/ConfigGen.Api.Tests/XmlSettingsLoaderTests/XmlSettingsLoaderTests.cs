...
namespace ConfigGen.Api.Tests.XmlSettingsLoaderTests
{
    internal class when_specifying_an_xml_settings_file_containing_two_configurations : GenerationServiceTestBase
    {
        Establish context = () =>
        {
            Assembly.GetExecutingAssembly().CopyEmbeddedResourceFileTo("TestResources.SimpleSettings.TwoConfigurations.TwoValues.xml", "App.Config.Settings.xml");
...
<xmlRoot>
  ...
  <Value2>Config2-Value2</Value2>
    ...
    internal class when_specifying_an_xml_settings_file_containing_two_configurations_with_the_settings_loader_type_option : GenerationServiceTestBase
    {
        Establish context = () =>
        {
            Assembly.GetExecutingAssembly().CopyEmbeddedResourceFileTo("TestResources.SimpleSettings.TwoConfigurations.TwoValues.xml", "App.Config.Settings.somefile");
            Assembly.GetExecutingAssembly().CopyEmbeddedResourceFileTo("TestResources.SimpleTemplate.TwoTokens.xml", "App.Config.Template.xml");

            PreferencesToSupplyToGenerator = new Dictionary<string, string>
            {
                {PreferenceNames.SettingsFilePath, "App.Config.Settings.somefile"},
                {PreferenceNames.SettingsFileType, "xml"}
            };
        };
